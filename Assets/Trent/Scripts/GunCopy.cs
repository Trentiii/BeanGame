...
public class GunCopy : MonoBehaviour
{
    //--Editable varibles--
    ...
    private GrappleCopy grappleRope; //Holds grappleRope script
    private SpringJoint2D springJoint2D; //Holds springJoint
    private Rigidbody2D rb2; //Holds rigibody2d
    private CopyMove pm; //Holds playerMovement script
    ...
    private void Start()
    {
        ...
        grappleRope = transform.GetChild(0).GetComponent<GrappleCopy>();
        springJoint2D = gunHolder.GetComponent<SpringJoint2D>();
        rb2 = gunHolder.GetComponent<Rigidbody2D>();
        mCamera = Camera.main;
        pm = gunHolder.GetComponent<CopyMove>();
